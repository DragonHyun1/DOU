    ...
    def _daq_monitoring_hardware_timed(self):
        ...
        try:
            ...
            if hasattr(self, 'daq_service') and self.daq_service:
                print("Starting DAQ hardware-timed CURRENT collection (33ms interval, 1000 samples avg, 10 seconds)...")
                
                daq_result = self.daq_service.read_current_channels_hardware_timed(
                    channels=enabled_channels,
                    sample_rate=30000.0,  # 30kHz (30 samples per ms, matches Manual tool)
                    compress_ratio=1000,  # 1000:1 compression (average 1000 samples → 1 per 33ms)
                    duration_seconds=10.0  # 10 seconds → 300k raw → 300 compressed
                )
