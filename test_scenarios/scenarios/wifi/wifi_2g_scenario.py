...
WiFi 2.4GHz 연결 후 15분간 전력 소비 측정
...
class WiFi2GScenario(BaseScenario):
    """WiFi 2G 15분 대기 test scenario implementation"""
    ...
    def get_config(self) -> TestConfig:
        """Get WiFi 2G test configuration"""
        config = TestConfig(
            ...
            test_duration=900.0  # 15분 테스트 (900초)
        )

        # Define detailed WiFi 2G test steps
        config.steps = [
            ...
            TestStep("enable_bluetooth", 5.0, "enable_bluetooth"),
            ...
            TestStep("wifi_2g_15min_wait", 900.0, "wait_15_minutes"),
    ...
    def execute_step(self, step: TestStep) -> bool:
        """Execute a single WiFi 2G test step"""
        try:
    ...
    def _step_enable_aod(self) -> bool:
        ...
        try:
            ...
            if success:
                self.log_callback("✅ AOD enabled", "info")
                return True
            else:
                self.log_callback("Failed to enable AOD", "error")
                return False
    ...
    def _step_connect_wifi_2g(self) -> bool:
        """Connect to WiFi 2.4GHz network"""
        try:
            self.log_callback("=== Connecting to WiFi 2G Network ===", "info")

            if not self.adb_service:
                self.log_callback("ADB service not available", "error")
                return False

            # Get WiFi 2G configuration
            wifi_config = WiFiConfig.get_2g_primary()
            ssid = wifi_config.get('ssid')
            password = wifi_config.get('password')

            self.log_callback(f"Connecting to: {ssid} ({wifi_config.get('frequency')})", "info")

            # Connect to WiFi 2G
            success = self.adb_service.connect_wifi_2g(ssid, password)

            if success:
                self.log_callback(f"✅ WiFi 2G connected: {ssid}", "info")
                return True
            else:
                self.log_callback(f"Failed to connect to WiFi 2G: {ssid}", "error")
                return False

        except Exception as e:
            self.log_callback(f"Error connecting to WiFi 2G: {e}", "error")
            return False
    ...
    def _step_wait_15_minutes(self) -> bool:
        ...
        try:
            self.log_callback("=== Waiting 15 minutes for WiFi 2G test (900 seconds) ===", "info")
            ...
            for i in range(total_seconds):
                if (i + 1) % update_interval == 0 or i == 0:
                    ...
                    seconds = (i + 1) % 60
                    self.log_callback(
                        f"WiFi 2G test progress: {i+1}/{total_seconds} seconds ({progress}%) - {minutes}m {seconds}s",
                        "info"
                    )
                time.sleep(1)

            self.log_callback("✅ 15-minute WiFi 2G test completed", "info")
            return True
    ...
    def _step_export_to_excel(self) -> bool:
        ...
        try:
            ...
            if data_count > 0:
                ...
                filename = f"wifi_2g_15min_test_{timestamp}.xlsx"
