...
class WiFi5GScenario(BaseScenario):
    """WiFi 5G 15분 대기 test scenario implementation"""
    ...
    def get_config(self) -> TestConfig:
        """Get WiFi 5G test configuration"""
        config = TestConfig(
        ...
        # Define detailed WiFi 5G test steps
        config.steps = [
            ...
            TestStep("wifi_5g_15min_wait", 900.0, "wait_15_minutes"),
    ...
    def execute_step(self, step: TestStep) -> bool:
        """Execute a single WiFi 5G test step"""
        try:
    ...
    def _step_connect_wifi_5g(self) -> bool:
        """Connect to WiFi 5GHz network"""
        try:
            self.log_callback("=== Connecting to WiFi 5G Network ===", "info")

            if not self.adb_service:
                self.log_callback("ADB service not available", "error")
                return False

            # Get WiFi 5G configuration
            wifi_config = WiFiConfig.get_5g_primary()
            ...
            if success:
                self.log_callback(f"✅ WiFi 5G connected: {ssid}", "info")
                return True
            else:
                self.log_callback(f"Failed to connect to WiFi 5G: {ssid}", "error")
                return False

        except Exception as e:
            self.log_callback(f"Error connecting to WiFi 5G: {e}", "error")
            return False
    ...
    def _step_wait_15_minutes(self) -> bool:
        ...
        try:
            self.log_callback("=== Waiting 15 minutes for WiFi 5G test (900 seconds) ===", "info")
            ...
            for i in range(total_seconds):
                if (i + 1) % update_interval == 0 or i == 0:
                    ...
                    seconds = (i + 1) % 60
                    self.log_callback(
                        f"WiFi 5G test progress: {i+1}/{total_seconds} seconds ({progress}%) - {minutes}m {seconds}s",
                        "info"
                    )
                time.sleep(1)

            self.log_callback("✅ 15-minute WiFi 5G test completed", "info")
            return True
    ...
    def _step_export_to_excel(self) -> bool:
        ...
        try:
            ...
            if data_count > 0:
                ...
                filename = f"wifi_5g_15min_test_{timestamp}.xlsx"
